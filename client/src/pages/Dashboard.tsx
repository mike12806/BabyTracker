...
export default function Dashboard() {
  ...
  const tiles: { cat: CategoryKey; label: string; last: string; detail: string; live?: boolean; onClick: () => void }[] = [
    {
      ...
      detail: lastFeeding ? `${lastFeeding.amount ? `${lastFeeding.amount} ${lastFeeding.amount_unit ?? "oz"}` : ""} ${prettifyType(lastFeeding.type)}`.trim() : "",
    ...
    },
    {
      ...
      detail: lastPump && lastPump.amount ? `${lastPump.amount} ${lastPump.amount_unit ?? "oz"}` : "",
  ...
  feedings.slice(0, 10).forEach((f) => allEvents.push({ cat: "feed", title: `${prettifyType(f.type)}${f.amount ? ` · ${f.amount} ${f.amount_unit ?? "oz"}` : ""}`, time: new Date(f.start_time).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" }), meta: formatDuration(f.start_time, f.end_time) }));
  ...
  pumpings.slice(0, 10).forEach((p) => allEvents.push({ cat: "pump", title: `Pump${p.amount ? ` · ${p.amount} ${p.amount_unit ?? "oz"}` : ""}`, time: new Date(p.start_time).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" }), meta: formatDuration(p.start_time, p.end_time) }));
