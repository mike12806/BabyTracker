...
const editedByExpr = (alias: string) =>
  `CASE WHEN ${alias}.updated_at != ${alias}.created_at THEN COALESCE(u2.name, u2.email) ELSE NULL END`;
    ...
    await Promise.all([
      c.env.DB.prepare(`
        SELECT 'Feeding' AS activity_type, f.id AS id, f.start_time AS event_time,
          ...
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("f")} AS edited_by
        FROM feedings f
        JOIN children c ON c.id = f.child_id
        LEFT JOIN users u ON u.id = f.created_by_user_id
        LEFT JOIN users u2 ON u2.id = f.updated_by_user_id
      ...
      c.env.DB.prepare(`
        SELECT 'Diaper Change' AS activity_type, d.id AS id, d.time AS event_time,
          ...
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("d")} AS edited_by
        FROM diaper_changes d
        JOIN children c ON c.id = d.child_id
        LEFT JOIN users u ON u.id = d.created_by_user_id
        LEFT JOIN users u2 ON u2.id = d.updated_by_user_id
      ...
      c.env.DB.prepare(`
        SELECT 'Sleep' AS activity_type, s.id AS id, s.start_time AS event_time,
          ...
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("s")} AS edited_by
        FROM sleep s
        JOIN children c ON c.id = s.child_id
        LEFT JOIN users u ON u.id = s.created_by_user_id
        LEFT JOIN users u2 ON u2.id = s.updated_by_user_id
      ...
      c.env.DB.prepare(`
        SELECT 'Tummy Time' AS activity_type, t.id AS id, t.start_time AS event_time,
          ...
          NULL AS amount, NULL AS amount_unit, NULL AS color,
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("t")} AS edited_by
        FROM tummy_time t
        JOIN children c ON c.id = t.child_id
        LEFT JOIN users u ON u.id = t.created_by_user_id
        LEFT JOIN users u2 ON u2.id = t.updated_by_user_id
      ...
      c.env.DB.prepare(`
        SELECT 'Pumping' AS activity_type, p.id AS id, p.start_time AS event_time,
          ...
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("p")} AS edited_by
        FROM pumping p
        JOIN children c ON c.id = p.child_id
        LEFT JOIN users u ON u.id = p.created_by_user_id
        LEFT JOIN users u2 ON u2.id = p.updated_by_user_id
      ...
      c.env.DB.prepare(`
        SELECT 'Note' AS activity_type, n.id AS id, n.time AS event_time,
          ...
          NULL AS amount, NULL AS amount_unit, NULL AS color,
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("n")} AS edited_by
        FROM notes n
        JOIN children c ON c.id = n.child_id
        LEFT JOIN users u ON u.id = n.created_by_user_id
        LEFT JOIN users u2 ON u2.id = n.updated_by_user_id
      ...
      c.env.DB.prepare(`
        SELECT 'Medication' AS activity_type, m.id AS id, m.time AS event_time,
          ...
          ${childNameExpr} AS child_name, ${loggedByExpr} AS logged_by, ${editedByExpr("m")} AS edited_by
        FROM medications m
        JOIN children c ON c.id = m.child_id
        LEFT JOIN users u ON u.id = m.created_by_user_id
        LEFT JOIN users u2 ON u2.id = m.updated_by_user_id
