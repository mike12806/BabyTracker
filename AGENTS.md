...
- **Read cache**: Cloudflare KV, bound as `CACHE`. D1 remains the source of truth for everything — nothing in KV is authoritative, and the namespace can be emptied at any moment without losing anything. The rule for what may go in it is not "is this read hot" but **"is an answer up to a minute old still a correct answer"**, because KV is eventually consistent. That rules out entry data (two caregivers looking at the same child is exactly the case it would get wrong) and user settings (changed by one person who expects to see it immediately); it rules *in* the four values in `src/kv/keys.ts`: the Cloudflare Access JWKS (a subrequest on every authenticated request, for a public document that rotates every few weeks), the caller's `users` row (an upsert plus a select ahead of every route handler), the boop line pool, and each child's daily note. Invalidation is the mechanism and the TTL is only the backstop: whatever writes the underlying row drops the key, and the TTL bounds the damage when that is missed. The binding is optional and every helper in `src/kv/cache.ts` swallows its own errors, so without KV — tests, local dev, a bad minute on Cloudflare's side — every read degrades to a miss and goes to D1 exactly as it did before. **KV migrations are a separate system from D1's**, and run *after* the deploy rather than before it; see the Database section below.
...
- The `CACHE` namespace is a read cache, never a store. Anything written there must be reconstructible from D1 (or Cloudflare Access) by the loader that wrote it
- All cache keys are built in `server/src/kv/keys.ts` and all of them sit behind `KV_SCHEMA_VERSION`. To change the shape of a stored value, bump that constant rather than writing a migration to rewrite the old ones — the deployed Worker stops reading them at the instant it goes live, which no delete sweep can race
- TTLs live in `server/src/kv/ttl.ts`, one named constant per cached value, and each one is documented by *how long that value may be wrong for*, not by how fresh would be nice. KV's own floor is 60 seconds; `cachePut` clamps to it
- Never let a KV failure fail a request. Everything in `server/src/kv/cache.ts` logs and degrades to a miss, and `Env.CACHE` is optional — the same convention as `LIVE` and the queues
- **KV migrations are applied automatically on every deploy**, by `scripts/kv-migrate.mjs`, *after* `wrangler deploy` rather than before it — nothing in the namespace is a source of truth, so a KV migration can only ever be a tidy-up after new code, never a precondition for it, and the version sweep would evict live keys if it ran first. Like D1's, merging the file is shipping it; never tell the user a KV migration "still needs applying"
- One-off migrations go in `server/kv-migrations/` as numbered `.mjs` files exporting `up({ kv, version, log })`, and must be safe to re-run — the ledger entry is only written once `up` resolves. The version sweep is deliberately not ledgered, so it runs again after the next bump. See `server/kv-migrations/README.md`
